...
export default function SingleSelectBoqItemButton({
  ...
  useEffect(() => {
    if (currentBoqLineIDs.length > 0 && boqLineValues.length > 0) {
      ...
      if (selectedBoqs.length > 0) {
        const infoText = `${selectedBoqs[0].item_number} ${selectedBoqs[0].item_name}`;
  ...
  const handleRadioToggle = (boqId: number) => {
  ...
  const selectedBoq =
    tempSelectedBoqIDs.length > 0
      ? boqLineValues.find((b) => b.id === tempSelectedBoqIDs[0])
      : null;
  ...
  const modalContent = isOpen && (
    <FormPopUp
      header={"SELECT BOQ ITEM"}
      setIsOpen={setIsOpen}
      handleSubmit={handleSubmit}
      addButtonLabel={"CONFIRM"}
      style={{ width: "75dvw", height: "95dvh" }}
      stickyFooter={
        totalPages > 1 || selectedBoq ? (
          <div
            ...
            {selectedBoq && (
              <div>
                ...
                <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                  ...
                  >
                    {selectedBoq.item_number} - {selectedBoq.item_name}
            ...
            <tbody>
              {paginatedItems.map((boq) => {
                ...
                return (
                  <tr
                    key={boq.id}
                    onClick={() => handleRadioToggle(boq.id)}
                    style={{ cursor: "pointer" }}
                  >
                    <td onClick={(e) => e.stopPropagation()}>
                      <input
                        type="radio"
                        name="boq-single-select"
                        checked={tempSelectedBoqIDs.includes(boq.id)}
                        onChange={() => handleRadioToggle(boq.id)}
                        style={{
                          width: "18px",
                          height: "18px",
                          cursor: "pointer",
