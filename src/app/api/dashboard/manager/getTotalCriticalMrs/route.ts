...
export async function POST(request: Request) {
  try {
    ...
    // Count overdue MRs at pre-LPO stages (before invoice/LPO segregation)
    const [mrCountRows]: any = await db.query(`
      SELECT COUNT(*) AS overdue_count
      FROM vw_mr_headers
      WHERE required_date < CURDATE()
        AND progress_id NOT IN (1, 14, 17, 25, 26)
    `);

    // Count overdue LPOs from MRs at post-invoice/LPO stages
    const [lpoCountRows]: any = await db.query(`
      SELECT COUNT(*) AS overdue_count
      FROM lpo l
      INNER JOIN mr_headers h ON h.id = l.mr_header_id
      WHERE h.progress_id IN (14, 17)
