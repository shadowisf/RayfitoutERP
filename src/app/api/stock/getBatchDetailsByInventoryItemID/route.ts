...
// Define the interface for batch details
interface BatchDetailsRow extends RowDataPacket {
  // Stock Entry Details
  stock_id: number;
  batch_id: number;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // First, get the stock entry with mr_header_id and mr_line_id
    const stockQuery = `
      SELECT 
        id,
        batch_id,
    ...
    // Now query everything using mr_header_id and mr_line_id
    const detailsQuery = `
      SELECT 
          -- Stock Entry Details
          s.id as stock_id,
    ...
    const [rows] = await db.query<BatchDetailsRow[]>(detailsQuery, [
      mr_header_id,
      mr_line_id,
    ]);
    ...
    if (rows.length > 0) {
      ...
      // MySQL automatically parses JSON fields, so we just need to return them
      const response = {
        ...batchDetails,
        // If they're strings, wrap in array; if already array, keep as-is; if null, return null
