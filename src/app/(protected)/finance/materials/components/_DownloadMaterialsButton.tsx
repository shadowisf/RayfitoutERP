...
export default function DownloadMaterialsButton({
  allRows,
  selectedKeys,
  categoryData,
  dateRange,
  ...
  const selectedRows = allRows.filter((r) =>
    selectedKeys.has(r.material_description),
  );
  ...
  const handleDownload = async (mode: "all" | "selected") => {
    ...
    if (!rows.length) {
      toast("No materials to export", "error");
      return;
    }
    ...
    try {
      ...
      link.href = url;
      link.download = `materials-${Date.now()}.pdf`;
