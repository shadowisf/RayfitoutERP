...
type Props = {
  ...
  onApplyFilters: (filters: VendorFilters) => void;
...
const PERFORMANCE_METRICS = [
  ...
  { key: "order_completion", label: "Order completion rate" },
  { key: "avg_delivery_time", label: "Avg. delivery time" },
  { key: "order_frequency", label: "Order frequency/week" },
];
...
// ── Dual range slider (matches transactions filter exactly) ───────────────────
...
// ── Main filter button ────────────────────────────────────────────────────────
export default function VendorsFilterButton({
  projects,
  currentFilters,
  onApplyFilters,
  spentBounds,
  ...
  const [localMetrics, setLocalMetrics] = useState<string[]>(
    currentFilters.selectedMetrics,
  );
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setLocalMetrics(currentFilters.selectedMetrics);
    }
  }, [isOpen]);
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {PERFORMANCE_METRICS.map(({ key, label }) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={localMetrics.includes(key)}
                    onChange={(e) => toggleMetric(key, e.target.checked)}
                    style={{
                      width: 18,
                      height: 18,
                      cursor: "pointer",
