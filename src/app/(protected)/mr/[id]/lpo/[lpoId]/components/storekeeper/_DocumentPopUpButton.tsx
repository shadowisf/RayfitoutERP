...
export default function DocumentsPopup({ lpoId }: DocumentsPopupProps) {
  const fileIcon = "/icons/folder.svg";
  ...
  function parseFileUrl(
    raw: string | string[] | null | undefined | any,
  ): string | null {
    if (!raw) return null;
  ...
  async function handleDownloadSignedLpo() {
    const signedUrl = parseFileUrl(lpoDetails?.signed_file);
    if (!signedUrl) {
      toast("No signed LPO available", "error");
      return;
    }
    try {
      await downloadFromUrl(
        signedUrl,
        `Signed-LPO-${String(lpoId).padStart(5, "0")}`,
      );
    } catch (err) {
      console.error("Error downloading signed LPO:", err);
  ...
  const hasLpo = !!parseFileUrl(lpoDetails?.signed_file);
  ...
  const documents = [
    {
      ...
      onClick: handleDownloadSignedLpo,
    },
    {
      key: "inv",
      icon: invIcon,
          ...
          >
            {documents.map((doc) => (
              <div
                key={doc.key}
                onClick={
                  doc.available && !doc.loading ? doc.onClick : undefined
                }
                style={{
                  display: "flex",
