...
import { BoqPDFWithNoPrice } from "../BoqPDFWithNoPrice";
...
type DownloadBoqWithNoPriceButtonProps = {
  boqHeader: BoqHeader;
  boqLines: GroupedBoqLines;
  children: React.ReactNode;
};

export default function DownloadBoqWithNoPriceButton({
  boqHeader,
  boqLines,
  children,
}: DownloadBoqWithNoPriceButtonProps) {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      const blob = await pdf(
        <BoqPDFWithNoPrice boqHeader={boqHeader} boqLines={processedLines} />
      ).toBlob();
      ...
      link.href = url;
      link.download = `BOQ-WOP-${String(boqHeader.id).padStart(5, "0")}.pdf`;
