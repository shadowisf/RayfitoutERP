...
import RestoreInventoryItemButton from "../[id]/components/_RestoreInventoryItemButton";
import { useAuth } from "@/app/context/AuthContext";
import RestoreAllInventoryItemButton from "../[id]/components/_RestoreAllIInventoryItemButton";
...
export default function InventoryArchive() {
  ...
  async function getInventoryItems() {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/inventory/getAllArchivedInventoryItems`,
        {
          method: "GET",
        },
      );
  ...
  return (
    <div className="dashboard">
      ...
      >
        <h1>
          <a href="/inventory">INVENTORY</a> &gt; ARCHIVE
        </h1>
        <RestoreAllInventoryItemButton />
      </div>
              ...
              <tbody>
                {currentItems.map((item, index) => {
                  ...
                  const stockStatus = isLoadingQuantities
                    ? getLoadingStatus()
                    : getStockStatus(availableQty, item.minimum_stock_quantity);
                      ...
                      <td>
                        <div style={{ display: "flex", gap: "10px" }}>
                          <InfoPopUpButton
                            ...
                            header={String(item.description).toUpperCase()}
                          />
