...
export default function UnarchiveMaterialButton({ item, onSuccess }: Props) {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/mr/getPredefinedItems`,
        {
          ...
          body: JSON.stringify({ id: item.id, is_archived: 0, changed_by: userInfo?.name ?? null }),
        },
      );
  ...
  const modal = isOpen && (
    ...
    >
      <p>
        Restore <strong>{item.material_description}</strong> back to the active
        material list?
      </p>
    </FormPopUp>
  );
